...
def analyzeFisherErrors(
    ...
    for n in np.arange(N):
        ...
        detected = np.where(networkSNR > detect_SNR[1])[0]

        return network_SNR[detected], parameter_errors[detected, :], sky_localization[detected]

def output_to_file(network_SNR, parameter_errors, sky_localization):
    ...
    save_data = np.c_[
        networkSNR, parameter_values.iloc, parameter_errors
    ]
    if signals_havesky:
