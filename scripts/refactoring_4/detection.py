...
class Detector:
    ...
    def __init__(self, name='ET', parameters=None, fisher_parameters=None, config=DEFAULT_CONFIG, plot=False, fmax=None):
        self.components = []
        ...
        fmax_ref = eval(str(detector_def['fmax']))
        if fmax is None:
            fmax = fmax_ref
        else:
            fmax = min(fmax, fmax_ref)
        if fmax < fmin:
            raise ValueError(f'{fmax=}, {fmin=}')
...
class Network:
    ...
    def __init__(self, detector_ids = None, detection_SNR=(0., 8.), parameters=None, fisher_parameters=None,
                 config=DEFAULT_CONFIG, plot=False, fmax=None):
        ...
        for d in np.arange(len(detector_ids)):
            detectors = Detector(name=detector_ids[d], parameters=parameters, fisher_parameters=fisher_parameters,
                                 config=config, plot=plot, fmax=fmax)
...
def projection_moon(parameters, detector, polarizations, timevector):
    ...
    before_obs_indices = np.where(timevector < tc - max_observation_time)
    proj[before_obs_indices, :] = 0.j
